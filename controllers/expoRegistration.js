...
exports.getExpoRegistrations = async (req, res) => {
  ...
  exports.exportExpoRegistrations = async (req, res) => {
    try {
      ...
      const registrations = await ExpoRegistration.find(filter).sort({ createdAt: -1 });
      ...
      const fileName = `Expo Registrations from ${new Date(from).toLocaleDateString()} to ${new Date(to).toLocaleDateString()}.xlsx`;
