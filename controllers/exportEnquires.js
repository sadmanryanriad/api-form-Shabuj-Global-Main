    ...
    try {
        ...
        enquires.forEach(enquire => {
            worksheet.addRow({
                ...
                notes: enquire.notes.map(note => `${note.note} (${new Date(note.timestamp).toLocaleString()})`).join(", "),
                status: enquire.status.map(s => `${s.status} (${new Date(s.timestamp).toLocaleString()})`).join(", ")
            });
        });

        // Generate buffer and send file for download
        res.setHeader(
            "Content-Type",
